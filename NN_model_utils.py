...
class NeuralNetwork(nn.Module):
    ...
    """
    def __init__(self, layers=[64, 64, 64], n_inputs=4, dropout=None):
        super().__init__()
...
class NeuralNetworkClassifier(BaseEstimator):
    ...
    def __init__(self, save_path=None, load=False, n_inputs=4,
        ...
        self.optimizer = optim.Adam(self.model.parameters(), betas=(momentum, 0.999), weight_decay=weight_decay, lr=float(lr))
    ...
    def fit(self, X, y, X_val=None, y_val=None, X_test=None, Y_test=None,
        ...
        if X_val is None and y_val is None:
            if self.val_split is None or not (self.val_split > 0.
                ...
                raise ValueError("val_split needs to be provided and lie "
        ...
        if X_test is not None: 
            test_loader = numpy_to_torch_loader(X_test, Y_test, device=self.device, batch_size=2048)
        # training loop
        self.model.train()
        for epoch in range(self.epochs if self.epochs is not None else 10000):
            ...
            if epoch == 0:
                ...
                max_SIC = np.array([epoch_max_SIC])
            else:
                train_losses = np.concatenate(
                    (train_losses, np.array([epoch_train_loss])))
                val_losses = np.concatenate(
                    (val_losses, np.array([epoch_val_loss])))
                max_SIC = np.concatenate(
                    (max_SIC, np.array([epoch_max_SIC])))
    ...
    def _max_SIC_path(self):
        return join(self.save_path, "CLSF_max_SIC.npy")
