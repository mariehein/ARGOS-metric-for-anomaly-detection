...
def plot_end_1D_multiple(ax, sig, ylim=None, title=None, loc="lower right"): 
	ax.legend(loc=loc)
	ax.set_ylabel(r"$\max\ \epsilon_S/\sqrt{\epsilon_B}$")
	ax.set_xlabel(r"$N_{sig}$")
	ax.set_xticks(sig)
...
def plot_optimized_runs(mode, signals, signals_plot, gen_direc, plotting_directory, rotated=False, metrics=["max_SIC", "val_loss", "val_SIC"], classifiers=["NN", "HGB", "AdaBoost"]):
	sic = np.zeros((len(signals),len(points)+2))
	sic_low = np.zeros((len(signals),len(points)+2))
	sic_upp = np.zeros((len(signals),len(points)+2))
	...
	for i,c in enumerate(classifiers): 
		...
		plot_end_1D_multiple(ax[i], signals_plot,title=c)
	fig.tight_layout()
	fig.savefig(plotting_directory+"1D_" + mode+"_"+c+"_optimized.pdf")
	plt.show()