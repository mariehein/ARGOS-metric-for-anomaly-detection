...
def classifier_training(X_train, Y_train, X_test, Y_test, args, run, X_eval=None, Y_eval=None, direc_run=None):
    ...
    for j in range(args.BDT_ensemble_over):
        print("Tree number:", args.BDT_ensemble_over*run+j)
        np.random.seed(run*args.BDT_ensemble_over+j+1)
        ...
        if args.use_half_statistics:
            eval_results += tree.predict_proba(X_eval)[:,1]/args.BDT_ensemble_over
        test_results += tree.predict_proba(X_test)[:,1]/args.BDT_ensemble_over
        del tree
        del results_f
    ...
    print("AUC last epoch: %.3f" % metrics.plot_roc(test_results, Y_test,title="BDT",directory=args.directory))
